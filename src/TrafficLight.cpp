...
void TrafficLight::cycleThroughPhases()
{
    ...
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
        long timeSinceLastSwitch = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - lastSwitch).count();
        if (timeSinceLastSwitch >= cycleDuration)
        {
